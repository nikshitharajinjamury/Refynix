...
const CodeComparison: React.FC<Props> = ({ original, optimized, language }) => {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-px bg-slate-800 h-full">
      ...
      <div className="flex flex-col h-full bg-[#0b0f1a] overflow-hidden border-l border-slate-800">
        <div className="px-5 py-3 border-b border-slate-800 bg-emerald-900/5 flex justify-between items-center">
          <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Optimized Rewrite</span>
          <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
        </div>
        <pre className="p-6 overflow-auto flex-1 code-font text-xs text-emerald-100 whitespace-pre leading-relaxed bg-emerald-950/5">
          {optimized}
        </pre>
      </div>
    </div>
